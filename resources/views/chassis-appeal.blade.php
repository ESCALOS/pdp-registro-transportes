...
<head>
    ...
    <title>Actualizar Documentos - {{ $chassis->license_plate }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            <h1>Actualizar Documentos del Chassis</h1>
            <p>{{ $chassis->license_plate }}</p>
        </div>
        <div class="content">
            @if(session('error'))
            ...
            <div class="chassis-info">
                <p><strong>Placa:</strong> {{ $chassis->license_plate }}</p>
                <p><strong>Empresa:</strong> {{ $chassis->company->business_name }}</p>
            </div>
            ...
            <form action="{{ route('chassis.appeal.update', $token) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
